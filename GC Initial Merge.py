...
stick_distance = 50 * AU   # !!!!! CHANGE FOR TESTING !!!!!------------------------------------
...
N_stars = int(1e4)         # Number of star particles per cluster
...
#stick_distance = schw
#print("Schwarzchild radius:", schw)
...
# Assign particle masses based on radial distance
...
m_stars = m_stars[:-1] # remove final star otherwise liats of different sizes when including BH

# Distribute particles in 3D space
x1_stars, y1_stars, z1_stars = distribute_particles(r_stars, N_stars, R)
...
m = np.concatenate([m_stars, [central_bh], m_stars, [central_bh]]) # len(m) = correct total for both GCs
...
for i in range(n):
    ...
    for idx in to_remove:
        m = np.delete(m, idx)  # Update the mass array
...
final_state_filename = 'merger 1.csv'
...
# If there are always exactly 2 BHs, you can separate their masses
if bh_masses.shape[1] == 2:
    bh1_masses = bh_masses[:, 0]
    bh2_masses = bh_masses[:, 1]
else:
    print("Number of BHs changed during the simulation.")
...
plt.title('Simulated Globular Cluster Merging')
plt.ylabel('Energy (J)')
plt.legend()

# Virial ratio plot (bottom subplot)
plt.subplot(2, 1, 2)
plt.plot(virial_ratio, label='Virial Ratio')
plt.axhline(y=1, color='r', linestyle='-', label='Equilibrium')
plt.title("Virial Ratio (2K/|U|) Evolution")
...
plt.plot(time_steps, bh1_masses / M_sun, label='BH 1 Mass')
plt.plot(time_steps, bh2_masses / M_sun, label='BH 2 Mass')
#plt.ylim(99.99,100.01)
